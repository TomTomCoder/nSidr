...
const NEXTJS_SOCKET_PORT = process.env.NEXTJS_SOCKET_PORT || process.env.SOCKET_PORT || '3001';
// IMPORTANT: REST API (NestJS) runs on a different port than the SockJS gateway.
// Launch script exports NEXTJS_BACKEND_PORT (typically 3002); the SOCKET_PORT
// fallback below is only safe when explicitly set to a REST-serving port.
// Without this, /api/* would route to the WS gateway and 500 on every request.
const NEXTJS_BACKEND_PORT = process.env.NEXTJS_BACKEND_PORT || process.env.BACKEND_PORT || '3002';
    ...
    directives: NEXT_BUILD_ENV_CSP
      ? {
          ...
          scriptSrc: [
            ...
            'https://*.teable.ai',
            'https://*.teable.cn',
            // App Builder preview iframe (srcDoc) loads React/Babel/Tailwind from CDN
            'https://unpkg.com',
...
const nextConfig = {
  ...
  async rewrites() {
    ...
    const docsProxy = {
      source: '/redocs',
      destination: `http://localhost:${NEXTJS_BACKEND_PORT}/redocs`,
    };

    // In dev, always proxy. In prod, normally a real reverse proxy fronts the app
    // and bakes-in rewrites are unwanted. But for local prod runs (pnpm start:prod:local
    // → launch-local.sh --prod), PROD_PROXY=1 is set so /api and /socket route through
    // next start to localhost:3002/3001. Without this branch the prod-local build hits
    // 404 on every API call, causing retry storms that block the main thread.
    if (isProd) {
      return process.env.PROD_PROXY === '1' ? [socketProxy, apiProxy, docsProxy] : [];
    }
    return [socketProxy, apiProxy, docsProxy];
  },
