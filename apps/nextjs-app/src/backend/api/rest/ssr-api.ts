...
export class SsrApi {
  ...
  async getTable(
    ...
    const viewsPromise = prefetchedViews
      ? Promise.resolve(prefetchedViews)
      : this.axios.get<IViewVo[]>(urlBuilder(GET_VIEW_LIST, { tableId })).then(({ data }) => data);

    const [fields, views, table] = await Promise.all([
      ...
      const recordsResult = await this.axios
        .get<IRecordsVo>(urlBuilder(GET_RECORDS_URL, { baseId, tableId }), {
          params: {
            viewId,
            fieldKeyType: FieldKeyType.Id,
            // Limit SSR seed records to 30 rows — covers the typical grid viewport
            // (~25 rows at default row height) without sending excess data.
            // The grid fetches more rows on demand as the user scrolls.
            take: 30,
