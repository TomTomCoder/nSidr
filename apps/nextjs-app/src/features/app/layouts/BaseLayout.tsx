...
const BaseLayoutInner: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const isEmbed = router.query.embed === '1' || router.query.embed === 'true';
    ...
    <ExpandRecordNavigationContext.Provider value={expandRecordNavValue}>
      ...
      >
        <div className="flex h-screen w-full">
          ...
          <div className="min-h-0 min-w-80 flex-1 overflow-hidden">{children}</div>
        </div>
        <UploadProgressPanel />
      </div>
      {!isEmbed && <LinkConnectorLine />}
      {!isEmbed && <UsageLimitModal />}
      {!isEmbed && <WorkFlowPanelModal />}
