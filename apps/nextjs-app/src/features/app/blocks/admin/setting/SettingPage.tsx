  ...
  return (
    <div className="flex h-screen flex-1 flex-col overflow-y-auto overflow-x-hidden p-4 sm:p-8">
      ...
      <AdminHelpPanel
        intro={
          <p>
            Cette page centralise la <strong>configuration de l&apos;instance</strong> Teable :
            accès utilisateur, email, domaine, branding et checklist de mise en production.
          </p>
        }
        steps={[
          {
            title: "Contrôler l'accès utilisateur",
            body: "Activez/désactivez l'inscription publique, les invitations dans les espaces et la création d'espaces. Activez la vérification email si vous souhaitez bloquer les comptes non vérifiés.",
          },
          {
            title: "Configurer l'email",
            body: 'Cliquez sur "Mail Config" pour renseigner votre serveur SMTP. Sans configuration email, les notifications et les automatisations email ne fonctionneront pas.',
          },
          {
            title: 'Vérifier le domaine',
            body: 'La section "Domain Verification" confirme que votre instance est accessible depuis l\'URL publique configurée dans l\'env var PUBLIC_ORIGIN.',
          },
          {
            title: 'Personnaliser le branding (EE)',
            body: 'En édition Enterprise, vous pouvez remplacer le nom et le logo "Teable" par votre propre marque dans la section Branding.',
          },
          {
            title: 'Valider la checklist',
            body: 'Le panneau de droite "Configuration" liste les prérequis de production (origine, proxy DB, email…). Un statut vert = correct, rouge = action requise.',
          },
        ]}
        tips={[
          {
            icon: '🆔',
            text: "Copiez l'Instance ID (en bas) pour les rapports de support Teable.",
          },
          {
            icon: '🔄',
            text: 'Les modifications sont appliquées immédiatement — pas besoin de redémarrer.',
          },
          {
            icon: '🌐',
            text: "PUBLIC_ORIGIN doit correspondre exactement à l'URL publique (avec https et sans slash final).",
          },
          {
            icon: '📬',
            text: 'Testez la config mail avec "Send test email" après avoir sauvegardé le SMTP.',
          },
        ]}
      />
      ...
      <div className="relative flex flex-1 flex-col overflow-hidden sm:flex-row">
        <div className="setting-page-left-container flex-1 overflow-y-auto overflow-x-hidden sm:pr-10">
          ...
          <div className="pb-6">
            <h2 className="mb-4 text-lg font-medium">Domain Verification</h2>
            <DomainVerification />
          </div>
