    ...
    <Form {...form}>
      ...
      {!!currentProvider && (
        <>
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                {/* GW-03: Ollama install link + Load models */}
                {type === LLMProviderType.OLLAMA && (
                  <div className="flex items-center gap-3 pt-1">
                    ...
                    >
                      {t('admin.setting.ai.installOllama' as never) || 'Install Ollama'} →
                    </a>
                    <OllamaLoadModelsButton
                      baseUrl={field.value || 'http://localhost:11434'}
                      onModelsLoaded={(names) => {
                        ...
                        const merged = [...existing, ...names.filter((n) => !existing.has(n))].join(
                          ', '
                        );
          ...
          {/* GW-05: Per-model API key overrides */}
          {form.watch('models') && (
            <ModelKeyOverrideSection
...
// ---------------------------------------------------------------------------
// GW-03: Ollama "Load models" button
// ---------------------------------------------------------------------------

interface IOllamaLoadModelsButtonProps {
  baseUrl: string;
  onModelsLoaded: (names: string[]) => void;
}

const OllamaLoadModelsButton = ({ baseUrl, onModelsLoaded }: IOllamaLoadModelsButtonProps) => {
  const { t } = useTranslation();
  ...
  const handleLoad = async () => {
    setLoading(true);
    try {
      const url = `/api/admin/setting/ollama-models?ollamaUrl=${encodeURIComponent(baseUrl || 'http://localhost:11434')}`;
      const res = await axios.get<{ models: Array<{ name: string }> }>(url);
      const names = (res.data?.models ?? []).map((m) => m.name).filter(Boolean);
      if (names.length === 0) {
        toast.info(
          t('admin.setting.ai.ollamaNoModels' as never) || 'No models found on Ollama instance'
        );
      } else {
        onModelsLoaded(names);
        toast.success(
          `${t('admin.setting.ai.ollamaLoadedModels' as never) || 'Loaded'} ${names.length} model(s) from Ollama`
        );
      }
    } catch {
      toast.error(
        t('admin.setting.ai.ollamaLoadFailed' as never) || 'Could not reach Ollama. Is it running?'
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <Button type="button" size="sm" variant="outline" onClick={handleLoad} disabled={loading}>
      {loading && <Loader2 className="mr-1 size-3 animate-spin" />}
      {t('admin.setting.ai.loadModels' as never) || 'Load models'}
    </Button>
  );
};

// ---------------------------------------------------------------------------
// GW-05: Per-model API key override section
// ---------------------------------------------------------------------------

interface IModelKeyOverrideSectionProps {
  models: string;
  modelConfigs: Record<string, IModelConfig>;
  onChange: (configs: Record<string, IModelConfig>) => void;
}

const ModelKeyOverrideSection = ({
  models,
  modelConfigs,
  onChange,
}: IModelKeyOverrideSectionProps) => {
  const { t } = useTranslation();
  const [expanded, setExpanded] = useState(false);
  ...
  return (
    <div className="space-y-2">
      ...
      >
        {expanded ? <ChevronUp className="size-4" /> : <ChevronDown className="size-4" />}
        {t('admin.setting.ai.perModelKeyOverrides' as never) || 'Per-model API key overrides'} (
        {modelList.length})
      </button>
      ...
      {expanded && (
        <div className="space-y-2 rounded-md border bg-muted/20 p-3">
          <p className="text-xs text-muted-foreground">
            {t('admin.setting.ai.perModelKeyDescription' as never) ||
              'Override the provider default API key for specific models.'}
          </p>
          {modelList.map((model) => {
            const hasOverride = Boolean(modelConfigs[model]?.apiKey);
            ...
            return (
              <ModelKeyOverrideRow
                ...
                isEditing={isEditing}
                onEdit={() => setEditingModel(isEditing ? null : model)}
                onSave={(apiKey) => {
                  onChange({
                    ...modelConfigs,
                    [model]: {
                ...
                onClear={() => {
                  ...
                  if (updated[model]) {
                    ...
                    const { apiKey: _removed, ...rest } = updated[model];
                    updated[model] = rest;
                  }
                  onChange(updated);
                }}
...
interface IModelKeyOverrideRowProps {
  model: string;
  hasOverride: boolean;
  isEditing: boolean;
  onEdit: () => void;
...
const apiKeyOverrideSchema = modelConfigSchema.pick({ apiKey: true });

const ModelKeyOverrideRow = ({
  model,
  hasOverride,
  isEditing,
  onEdit,
  onSave,
  onClear,
}: IModelKeyOverrideRowProps) => {
  const { t } = useTranslation();
  const overrideForm = useForm<{ apiKey: string }>({
    resolver: zodResolver(apiKeyOverrideSchema),
    defaultValues: { apiKey: '' },
  });

  const handleSubmit = overrideForm.handleSubmit((data) => {
    onSave(data.apiKey || '');
  });
  ...
  return (
    <div className="space-y-1">
      <div className="flex items-center gap-2">
        <span className="flex-1 truncate text-sm" title={model}>
          {model}
        </span>
        <Badge variant={hasOverride ? 'default' : 'secondary'} className="text-xs">
          {hasOverride
            ? t('admin.setting.ai.overrideSet' as never) || 'Override set'
            : t('admin.setting.ai.usingProviderDefault' as never) || 'Using provider default key'}
        </Badge>
        {hasOverride && (
          <Button type="button" size="sm" variant="ghost" onClick={onClear} className="h-6 px-2">
            <X className="size-3" />
          </Button>
        )}
        <Button type="button" size="sm" variant="outline" onClick={onEdit} className="h-6 px-2">
          {isEditing
            ? t('actions.cancel') || 'Cancel'
            : t('admin.setting.ai.override' as never) || 'Override'}
        </Button>
      </div>
      ...
      {isEditing && (
        <form onSubmit={handleSubmit} className="flex gap-2 pl-2">
          <Input
            type="password"
            placeholder={t('admin.setting.ai.apiKey') || 'API key'}
            className="h-7 text-xs"
            {...overrideForm.register('apiKey')}
          />
...
// ---------------------------------------------------------------------------
// GW-04: Embedding provider picker (separate export for use in settings page)
// ---------------------------------------------------------------------------

interface IEmbeddingProviderPickerProps {
  /** All configured LLM providers (to filter for embedding-capable models) */
  providers: LLMProvider[];
  /** Current embedding provider config from aiConfig.embeddingProvider */
  value: IAIConfig['embeddingProvider'] | null | undefined;
  /** Callback to persist the updated embeddingProvider */
  onChange: (embeddingProvider: IAIConfig['embeddingProvider'] | null) => void;
}

/**
 * GW-04: Embedding provider + model picker.
 * Filters provider models that are embedding-capable (modelType === 'embedding' tag in
 * modelConfigs, or all models from an Ollama provider which can serve embeddings).
 * Falls back to showing all models if no embedding-specific models are flagged.
 *
 * Exported separately so it can be placed in the admin AI settings wizard.
 */
export const EmbeddingProviderPicker = ({
  providers,
  value,
  onChange,
}: IEmbeddingProviderPickerProps) => {
  const { t } = useTranslation();

  // Build a flat list of { label, providerType, providerName, modelId } options
  const options = useMemo(() => {
    ...
    for (const provider of providers) {
      ...
      for (const modelId of modelList) {
        const cfg = provider.modelConfigs?.[modelId];
        // Include if: modelType is 'embedding', OR no modelType is set (include all — let user decide)
        const isEmbeddingCapable = !cfg?.modelType || cfg.modelType === 'embedding';
        if (isEmbeddingCapable) {
          list.push({
  ...
  const selectedKey = value ? `${value.providerType}|${value.providerName}|${value.modelId}` : '';
  ...
  const handleChange = (key: string) => {
    ...
    const [providerType, providerName, modelId] = key.split('|');
    onChange({ providerType: providerType as LLMProviderType, providerName, modelId });
  };

  if (options.length === 0) {
    return (
      <div className="rounded-md border bg-muted/20 p-3 text-sm text-muted-foreground">
        {t('admin.setting.ai.noEmbeddingModels' as never) ||
          'No embedding-capable models found. Configure a provider with embedding models first.'}
      </div>
    );
  }
  ...
  return (
    <div className="space-y-2">
      <div>
        <FormLabel>
          {t('admin.setting.ai.embeddingProvider' as never) || 'Embedding provider'}
        </FormLabel>
        <FormDescription>
          {t('admin.setting.ai.embeddingProviderDescription' as never) ||
            'Select the provider and model used for document embeddings (knowledge search, etc.).'}
        </FormDescription>
      </div>
      <Select value={selectedKey} onValueChange={handleChange}>
        <SelectTrigger>
          <SelectValue
            placeholder={
              t('admin.setting.ai.selectEmbeddingModel' as never) || 'Select an embedding model…'
            }
          />
        </SelectTrigger>
        <SelectContent>
          {options.map((opt) => (
            <SelectItem
              key={`${opt.providerType}|${opt.providerName}|${opt.modelId}`}
              value={`${opt.providerType}|${opt.providerName}|${opt.modelId}`}
            >
              {opt.label}
            </SelectItem>
          ))}
      ...
      {value && (
        ...
        >
          ...
          {t('admin.setting.ai.clearEmbeddingProvider' as never) || 'Clear embedding provider'}
        </Button>
      )}
    </div>
  );
