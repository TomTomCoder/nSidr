...
export function useGatewayModels({
  ...
  const {
    ...
    // Always fetch; react-query deduplicates concurrent calls and caches the result.
    // staleTime prevents re-fetching for 5 minutes — gateway model list rarely changes.
    // Previously this was gated on `needGroup`, which prevented the model browser
    // in AiSettingsHub (where needGroup is undefined) from ever loading models.
    enabled: true,
    staleTime: 5 * 60 * 1000,
  });
