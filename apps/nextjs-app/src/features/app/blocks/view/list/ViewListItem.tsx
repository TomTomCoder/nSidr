...
const ViewListItemInner: React.FC<IProps> = ({ view, removable, isActive, onEdit }) => {
      ...
      <ContextMenu>
        <ContextMenuTrigger asChild disabled={isEditing || isReadOnlyPreview}>
          <div
            ...
            tabIndex={0}
            className={cn(
              'flex h-7 max-w-52 items-center overflow-hidden rounded-md p-1 text-sm transition-colors hover:bg-accent',
              {
                'bg-[linear-gradient(135deg,rgba(124,58,237,0.12),rgba(79,70,229,0.08),rgba(14,165,233,0.08))] shadow-[inset_0_0_0_1px_rgba(124,58,237,0.25)]':
          ...
          >
            <Button
              ...
              className={cn('m-0 flex w-full rounded-sm hover:bg-transparent p-0', {
                'bg-transparent text-foreground': isActive && !isHighlighted,
...
// Memoize so that sibling re-renders (e.g. another view item changing state, or
// a non-view related parent re-render during the table-switch sequence) do not
// force every list item to re-run its ~10 hooks + heavy ContextMenu subtree.
export const ViewListItem = memo(ViewListItemInner);
