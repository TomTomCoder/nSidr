...
export const FormBody = (props: IFormBodyProps) => {
  ...
  const [errors, { set: setError, remove: removeError, reset: resetErrors }] = useMap<
  ...
  const onChange = (fieldId: string, value: unknown) => {
    if (errors[fieldId] && value != null && value != '') {
      removeError(fieldId);
    }
  ...
  const onVerify = () => {
    ...
    visibleFields.forEach((field) => {
      ...
      const required = field.notNull || columnMeta[field.id]?.required;
      const isEmpty = value == null || value === '';

      if (isEmpty) {
        if (!required) return;
        if (!firstErrorFieldId) firstErrorFieldId = field.id;
        setError(field.id, 'required');
        return;
      }

      // Type-level validation: native per-FieldType Zod schema (cellValueValidation).
      // ponytail: native cell-value validator already covers email/url/number/date/link/etc.
      const result = field.validateCellValue(value);
      if (result && !result.success) {
        if (!firstErrorFieldId) firstErrorFieldId = field.id;
        setError(field.id, result.error.issues[0]?.message ?? 'invalid');
      }
    });
