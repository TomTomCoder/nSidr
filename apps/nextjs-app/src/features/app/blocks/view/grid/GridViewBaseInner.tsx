  ...
  const onContextMenu = (selection: CombinedSelection, position: IPosition) => {
    ...
    if (isCellSelection || isRowSelection) {
      ...
      if (isMultipleSelected) {
        ...
        // Phase 16-04: resolve the right-clicked cell's field for the AI
        // "Regenerate" menu gate. Only populated for single-cell selection
        // (isCellSelection); falls back to undefined for row-only selection.
        const colIndex = isCellSelection ? ranges[0][0] : undefined;
        const cellFieldInstance =
          colIndex !== undefined && colIndex >= 0 ? fields[colIndex] : undefined;
        const cellField = cellFieldInstance
          ? { id: cellFieldInstance.id, type: cellFieldInstance.type }
          : undefined;
        const cellRecordId = record?.id;
