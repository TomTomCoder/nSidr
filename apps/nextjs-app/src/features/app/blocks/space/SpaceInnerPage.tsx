...
export const SpaceInnerPage: React.FC = () => {
  ...
  const [settingTab, setSettingTab] = useState<(typeof SettingTab)[keyof typeof SettingTab]>(
    SettingTab.Plan
  );
  ...
  useEffect(() => {
    const { subscribeLevel, setting } = router.query;
    if (subscribeLevel && isCloud && space?.role === Role.Owner) {
      setSettingModalOpen(true);
      return;
    }
    if (setting && space?.role === Role.Owner) {
      const tabMap: Record<string, (typeof SettingTab)[keyof typeof SettingTab]> = {
        general: SettingTab.General,
        collaborator: SettingTab.Collaborator,
        extensions: SettingTab.Extensions,
        plan: SettingTab.Plan,
      };
      const mapped = tabMap[setting as string];
      if (mapped) {
        setSettingTab(mapped);
        setSettingModalOpen(true);
      }
    }
