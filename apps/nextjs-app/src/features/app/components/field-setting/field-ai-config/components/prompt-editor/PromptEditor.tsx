...
export const PromptEditor = ({
  ...
  // Stable refs for callbacks that shouldn't trigger extensions recreate
  const onChangeRef = useRef(onChange);
  const decorateFieldsRef = useRef(decorateFields);
  useEffect(() => {
    onChangeRef.current = onChange;
  }, [onChange]);
  useEffect(() => {
    decorateFieldsRef.current = decorateFields;
  }, [decorateFields]);
    ...
    return [
      ...
      EditorView.updateListener.of((update) => {
        if (update.docChanged) {
          ...
          decorateFieldsRef.current(update.view);
