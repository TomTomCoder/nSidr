...
export const useChatPanelStore = create<IChatPanelState>()(
  persist(
    ...
    }),
    {
      ...
      partialize: (state) => ({
        // Never persist 'expanded' — it should not survive a page reload
        status: state.status === 'expanded' ? 'open' : state.status,
      }),
      onRehydrateStorage: () => (state) => {
        // Migrate any stored 'expanded' to 'open' on load
        if (state && state.status === 'expanded') {
          state.status = 'open';
        }
      },
    }
