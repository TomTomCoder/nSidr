...
/**
 * Maximum number of snapshot IDs to fetch in a single batch during getOpsBulk.
 * Caps peak heap usage when a large table subscribes on initial connection.
 */
...
export class ShareDbAdapter extends ShareDb.DB {
  ...
  async internalGetOps(
    ...
    try {
      ...
      const targetVersion = to !== null ? Math.min(to, baseRaw.v + 1) : baseRaw.v + 1;
      const gapVersion = Math.max(targetVersion - from, 0);

      // Guard against unbounded op array allocation.
      //
      // The DB never stores historical ops — only the current snapshot. When a client
      // requests ops over a large version range (e.g. from=0, to=null for a document
      // at version 50 000), naively allocating new Array(50000) here causes OOM during
      // initial ShareDB subscription sync for large tables.
      //
      // All intermediate ops are empty shells that only advance the client's version
      // counter. Only the final op carries the actual diff (editOp). When the gap is
      // large we collapse intermediate empty ops into a single placeholder followed by
      // the content op, which is functionally equivalent for the client.
      if (gapVersion <= 1) {
        // Zero or one op — emit directly without intermediate shells
        const editOps: IEditOp[] = [];
        if (gapVersion === 1) {
          editOps.push({
            ...baseRaw,
            src: getRandomString(21),
      ...
      // Two ops: one empty placeholder to advance the version from `from` to `version`,
      // then one content op at `version`. This caps allocation at O(1) regardless of
      // how large the version gap is.
      const editOps: IEditOp[] = [
        {
          ...baseRaw,
          src: getRandomString(21),
  ...
  async getOpsBulk(
    ...
    // Load snapshot data in batches to cap peak heap usage.
    //
    // Previously, all N snapshot IDs were fetched in a single call. For large tables
    // (e.g. 10 000 records), this loaded the entire table's field data into one Map
    // that persisted for the full duration of the sequential processing loop — causing
    // OOM on initial ShareDB subscription sync. Batching limits the live set of
    // snapshot objects in memory at any one time.
    const snapshotDataMap: Record<string, ISnapshotBase<unknown>> = {};
    for (let i = 0; i < needGetSnapshotDataIds.length; i += SNAPSHOT_BATCH_SIZE) {
      const batch = needGetSnapshotDataIds.slice(i, i + SNAPSHOT_BATCH_SIZE);
      const batchSnapshots = await this.getSnapshotData(
        docType as IdPrefix,
        collectionId,
        batch,
        options
      ...
      for (const snapshot of batchSnapshots) {
        snapshotDataMap[snapshot.id] = snapshot;
      }
    }
