...
export class V2ContainerService implements OnApplicationBootstrap, OnModuleDestroy {
  ...
  async onApplicationBootstrap(): Promise<void> {
    ...
    // Fire-and-forget warm-up: the first record mutation otherwise pays for
    // lazy pool connections + bus/repository DI graph instantiation in-request
    // (measured ~6s cold vs ~80ms warm).
    void this.warmUp(container).catch((e) =>
      this.logger.warn(`V2 warm-up failed (non-fatal): ${(e as Error).message}`)
    );
  }

  private async warmUp(container: DependencyContainer): Promise<void> {
    const started = Date.now();
    // Open one connection on each pool so the first request doesn't pay TCP/TLS/auth.
    const { sql } = await import('kysely');
    type AnyDb = import('kysely').Kysely<unknown>;
    const metaDb = container.resolve<AnyDb>(v2MetaDbTokens.db);
    const dataDb = container.resolve<AnyDb>(v2DataDbTokens.db);
    await Promise.all([sql`SELECT 1`.execute(metaDb), sql`SELECT 1`.execute(dataDb)]);
    // Instantiate the command/query bus graphs (tsyringe resolves lazily).
    container.resolve(v2CoreTokens.commandBus);
    container.resolve(v2CoreTokens.queryBus);
    this.logger.log(`V2 warm-up completed in ${Date.now() - started}ms`);
  }
    ...
    const container = await createV2NodePgContainer({
      ...
      computedUpdate:
        computedUpdateMode === 'sync'
          ? { mode: 'sync', pollingConfig: { enabled: false } }
          : undefined,
