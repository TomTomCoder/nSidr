...
export const modelProviders = {
  ...
  [LLMProviderType.KILOCODE]: createOpenAICompatibleWrapper,
  // AI_GATEWAY is handled separately in ai.service.ts using createGateway from 'ai'
} as const;

/**
 * Well-known public API endpoints per provider. Teable requires a baseUrl to
 * instantiate a standard provider, but BYOK configs frequently omit it for
 * providers whose SDK has a stable default endpoint. We supply the default so
 * the model resolves instead of throwing "AI configuration is not set".
 * Only applied when the provider config has no explicit baseUrl.
 */
export const PROVIDER_DEFAULT_BASE_URL: Partial<Record<LLMProviderType, string>> = {
