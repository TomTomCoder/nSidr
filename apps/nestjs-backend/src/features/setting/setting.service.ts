...
/**
 * SSRF blocklist for the Ollama proxy (T-15-04).
 * Module-level constant so unit tests using Object.create() can iterate it.
 *
 * Blocked:
 * - AWS/GCP/Azure/Alibaba instance-metadata endpoints (169.254.x.x link-local)
 * - Known Kubernetes / Docker-internal hostnames
 * - Common cloud metadata service hostnames
 */
const SSRF_BLOCKLIST: RegExp[] = [
  // APIPA / link-local (all major cloud metadata endpoints sit on 169.254.169.254)
  /^169\.254\./,
  // Kubernetes API server / service-account metadata
  /^kubernetes\.default(\.svc(\.cluster\.local)?)?$/i,
  // Docker internal gateway
  /^docker\.internal$/i,
  // Google Cloud metadata
  /^metadata\.google\.internal$/i,
  // Generic metadata hostnames used by several cloud providers
  /^metadata\.internal$/i,
];
...
export class SettingService {
  ...
  // ---- Ollama /api/tags proxy (GW-02, GW-03, T-15-04) ----

  /**
   * Validate that `rawUrl` is safe to proxy server-side (SSRF mitigation, T-15-04).
   * Throws BadRequestException if the URL is suspicious.
   */
  private assertSsrfSafe(rawUrl: string): URL {
    let parsed: URL;
    try {
      parsed = new URL(rawUrl);
    } catch {
      throw new BadRequestException(`Invalid Ollama URL: ${rawUrl}`);
    }

    // Only allow http and https
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
      throw new BadRequestException(
        `Ollama URL must use http or https scheme, got: ${parsed.protocol}`
      );
    }

    const host = parsed.hostname.toLowerCase();

    // Block link-local (169.254.x.x) — these are ALWAYS metadata endpoints
    for (const pattern of SSRF_BLOCKLIST) {
      if (pattern.test(host)) {
        throw new BadRequestException(
          `Ollama URL host '${host}' is blocked for security reasons (SSRF protection). ` +
            'Use a publicly reachable hostname or the configured Ollama host.'
        );
      }
    }
  ...
  /**
   * Proxy GET {ollamaUrl}/api/tags server-side to avoid CORS and keep config server-side.
   * Returns the parsed Ollama model list `{ models: [{ name, ... }] }`.
   *
   * SSRF mitigation (T-15-04): URL is validated before the fetch; link-local and
   * known metadata endpoints are rejected.
   *
   * @param ollamaUrl - Base URL of the local Ollama instance. Defaults to
   *   `http://localhost:11434`.
   */
  async listOllamaModels(
    ollamaUrl: string = 'http://localhost:11434'
  ): Promise<{ models: Array<{ name: string; modified_at?: string; size?: number }> }> {
    const safe = this.assertSsrfSafe(ollamaUrl);
    const tagsUrl = `${safe.origin}/api/tags`;

    try {
      const res = await fetch(tagsUrl, { signal: AbortSignal.timeout(10_000) });
      if (!res.ok) {
      ...
      this.logger?.warn(
        `[listOllamaModels] Failed to reach Ollama at ${tagsUrl}: ${
          error instanceof Error ? error.message : String(error)
        }`
      );
      throw new BadRequestException(
        `Could not reach Ollama at ${safe.origin}. ` +
