...
import { getConfiguredProviders } from '../../user-integration/user-integration.constants';
...
import { generateText, tool, generateImage } from 'ai';
...
import { AiService, INSTANCE_PROVIDER_NAME } from '../../ai/ai.service';
...
export class SettingOpenApiService {
  ...
  > {
    ...
    // Advertise every user-integration provider that has client credentials configured.
    const availableIntegrationProviders: string[] = getConfiguredProviders();
  ...
  async testLLM(testLLMRo: ITestLLMRo): Promise<ITestLLMVo> {
    ...
    try {
      ...
      const model = incomingModelKey ? this.parseModelKey(incomingModelKey).model : modelArray[0];

      const modelKey = `${type}@${model}@${name}`;
      // Build an ad-hoc provider list from the test request so the gateway can resolve creds
      const llmProviders = [{ type, name, apiKey, baseUrl, models }];

      // Handle AI Gateway separately — use ad-hoc helper (stored apiKey path uses getModelInstance)
      if (type === LLMProviderType.AI_GATEWAY) {
        ...
        // Standard text model testing via ad-hoc gateway (test credentials, not stored)
        const testPrompt = 'Hello, please respond with "Connection successful!"';
        const modelInstance = this.aiService.getAdHocGatewayModelInstance(
          apiKey,
          model,
          baseUrl || undefined
      ...
      if (shouldTestImageGeneration) {
        // Google uses generateText (not generateImage), so pass a text model instance
        if (type === LLMProviderType.GOOGLE) {
          const textModelInstance = await this.aiService.getModelInstance(modelKey, llmProviders);
          return await this.testImageGenerationModel(
            textModelInstance as unknown as import('ai').ImageModel,
            model,
            type,
            testImageToImage
          );
        }
        const imageModelInstance = await this.aiService.getModelInstance(
          modelKey,
          llmProviders,
          true
        );
      ...
      // Standard text model testing via gateway
      const testPrompt = 'Hello, please respond with "Connection successful!"';
      ...
      const supportAbilities = await this.testChatModelAbility(
  ...
  private async testImageGenerationModel(
    imageModelInstance: import('ai').ImageModel,
    model: string,
    providerType: LLMProviderType,
    ...
    try {
      ...
      if (providerType === LLMProviderType.GOOGLE) {
        return await this.testGoogleImageGeneration(
          imageModelInstance as unknown as LanguageModel,
          testImageToImage
        );
      }

      // OpenAI-style image generation (DALL-E, GPT Image, etc.)
      const imageModel = imageModelInstance;
  ...
  private async testSingleModel(
    ...
    try {
      ...
      // Build ad-hoc provider list from the batch test provider record
      const llmProviders = [{ type, name: providerName, apiKey, baseUrl, models: model }];

      // Handle AI Gateway separately — use ad-hoc helper (stored apiKey path uses getModelInstance)
      if (type === LLMProviderType.AI_GATEWAY) {
        modelInstance = this.aiService.getAdHocGatewayModelInstance(
          apiKey,
          model,
          baseUrl || undefined
        ) as unknown as LanguageModel;
      } else {
        modelInstance = (await this.aiService.getModelInstance(
          modelKey,
          llmProviders
        )) as unknown as LanguageModel;
      }
  ...
  private async testAttachmentTransferModes(
    ...
    try {
      // Build model instance via ad-hoc helper (user-provided key, not stored settings)
      const modelInstance = this.aiService.getAdHocGatewayModelInstance(
        apiKey,
        testModel,
        baseUrl || undefined
  ...
  private async testAiGatewayKey(apiKey: string, baseUrl?: string): Promise<ITestApiKeyVo> {
    try {
      // Use ad-hoc helper — user-provided key, not stored settings
      const modelInstance = this.aiService.getAdHocGatewayModelInstance(
        apiKey,
        'openai/gpt-4o-mini',
        baseUrl
  ...
  /**
   * Proxy GET {ollamaUrl}/api/tags server-side (GW-03, T-15-04 SSRF mitigation).
   * Returns the parsed Ollama model list.
   *
   * @param ollamaUrl - Base URL of the Ollama instance (defaults to http://localhost:11434)
   */
  async listOllamaModels(
    ollamaUrl?: string
  ): Promise<{ models: Array<{ name: string; modified_at?: string; size?: number }> }> {
    return this.settingService.listOllamaModels(ollamaUrl);
  }
}
