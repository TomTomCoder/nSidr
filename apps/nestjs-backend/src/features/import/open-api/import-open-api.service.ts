...
import { IMPORT_QUEUE, type ImportJobData } from '../../queue/queue.types';
...
export class ImportOpenApiService {
  ...
  constructor(
    ...
    @InjectQueue(IMPORT_QUEUE) private readonly importQueue: Queue<ImportJobData>,
  ...
  async createTableFromImport(baseId: string, importRo: IImportOptionRo, maxRowCount?: number) {
    ...
    // Dispatch outer import job to IMPORT_QUEUE for visibility and tracking (D-05).
    // The actual import work is handled by the existing TABLE_IMPORT_CSV_CHUNK_QUEUE sub-queues.
    try {
      await this.importQueue.add('process-import', {
        importId: getRandomString(12),
        tableId: Object.keys(worksheets)[0] ?? 'unknown',
        userId,
      } satisfies ImportJobData);
    } catch (e) {
      this.logger.warn(
        '[ImportOpenApiService] Failed to dispatch to IMPORT_QUEUE (non-blocking)',
        e
      );
    }
