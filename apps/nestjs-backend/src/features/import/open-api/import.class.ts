...
export abstract class Importer {
  ...
  async genColumns() {
    ...
    for (const [sheetName, cols] of Object.entries(parseResult)) {
      ...
      // First 5 data rows (skip header row at index 0)
      const dataRows = cols.slice(1, 6);
      const sampleRows = dataRows.map((row) =>
        (row as unknown[]).map((cell) => (cell === '' || cell == null ? null : cell))
      );
...
// TSV files use the same parser as CSV — PapaParse auto-detects the tab delimiter.
// This class exists to provide explicit routing and type validation for .tsv files.
export class TsvImporter extends CsvImporter {}
...
export const importerFactory = (type: SUPPORTEDTYPE, config: IImportConstructorParams) => {
  switch (type) {
    ...
    case SUPPORTEDTYPE.TSV:
      return new TsvImporter(config);
