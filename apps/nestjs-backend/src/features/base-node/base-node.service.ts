...
export class BaseNodeService {
  ...
  protected async getNodeResource(
    ...
    switch (type) {
      ...
      case BaseNodeResourceType.Agent:
  ...
  protected async getAppResources(
    baseId: string,
    ids?: string[]
  ...
  protected async getWorkflowResources(
    baseId: string,
    ids?: string[]
  ): Promise<{ id: string; name: string }[]> {
  ...
  protected async createResource(
    ...
    switch (resourceType) {
      ...
      case BaseNodeResourceType.App: {
        const app = await this.appBuilderService.createApp(baseId, ro.name ?? 'New App');
        return { id: app.id, name: app.name };
      }
      case BaseNodeResourceType.Workflow: {
        const workflow = await this.workflowService.createWorkflow(
          baseId,
          ro.name ?? 'New Automation'
        );
        return { id: workflow.id, name: workflow.name };
      }
      case BaseNodeResourceType.Agent: {
        // Agent is pre-created; resourceId contains the existing agent's ID
        const existingId = (ro as Record<string, unknown>).resourceId as string;
        if (!existingId) {
          throw new CustomHttpException(
            'resourceId required for Agent base-node',
            HttpErrorCode.VALIDATION_ERROR,
            { localization: { i18nKey: 'httpErrors.baseNode.invalidResourceType' } }
          );
        }
        return { id: existingId, name: ro.name };
      }
      default:
        throw new CustomHttpException(
  ...
  protected async updateResource(
    ...
    switch (type) {
      ...
      case BaseNodeResourceType.App:
        if (name) {
          await this.appBuilderService.renameApp(baseId, id, name);
        }
        break;
      case BaseNodeResourceType.Workflow:
        if (name) {
  ...
  protected async deleteResource(
    ...
    switch (type) {
      ...
      case BaseNodeResourceType.Dashboard:
        ...
        break;
      case BaseNodeResourceType.App:
        await this.appBuilderService.deleteApp(baseId, id);
        break;
      case BaseNodeResourceType.Workflow:
        await this.workflowService.deleteWorkflow(baseId, id);
        break;
      case BaseNodeResourceType.Agent:
        await this.prismaService.agent.delete({ where: { id } });
        break;
      default:
        throw new CustomHttpException(
          `Invalid resource type ${type}`,
  ...
  /**
   * Context graph: every resource node in the base + every field-level link
   * (Reference rows) between this base's fields. Consumed by AI grounding and
   * the graph viewer.
   *
   * ponytail: flat findMany over already-indexed columns; the base_node tree
   * stays one level deep in practice, so no recursive CTE.
   */
  async getContextGraph(baseId: string): Promise<{
    nodes: { id: string; parentId: string | null; resourceType: string; resourceId: string }[];
    fields: { id: string; tableId: string; name: string; type: string }[];
    edges: { fromFieldId: string; toFieldId: string }[];
  }> {
    const nodes = await this.prismaService.baseNode.findMany({
      where: { baseId },
    ...
    if (tableIds.length === 0) return { nodes, fields: [], edges: [] };
