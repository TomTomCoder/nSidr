...
export class RecordService {
  ...
  async getDocIdsByQuery(
    ...
    const _tPerf3 = performance.now();

    // PERF: getSearchHitIndex() returns null immediately when there is no search term,
    // but the second permission wrapView + builder work below ran unconditionally on every
    // grid load. Skip the whole search block when there is nothing to search.
    if (!query.search) {
      this.logger.debug(
        `[doc-ids perf] group=${(_tPerf1 - _tPerf0).toFixed(0)}ms ` +
          `build=${(_tPerf2 - _tPerf1).toFixed(0)}ms rawq=${(_tPerf3 - _tPerf2).toFixed(0)}ms ` +
          `(rows=${ids.length}, no-search) total=${(_tPerf3 - _tPerf0).toFixed(0)}ms`
      );
