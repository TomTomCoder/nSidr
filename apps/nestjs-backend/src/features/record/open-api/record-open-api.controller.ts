...
import { AiCellRegenerateService } from '../../ai/ai-cell-regenerate.service';
...
export class RecordOpenApiController {
  constructor(
    ...
    private readonly aiCellRegenerateService: AiCellRegenerateService
  ) {}
  ...
  @Permissions('record|update')
  @Post(':recordId/:fieldId/regenerate')
  async regenerateAiCell(
    @Param('tableId') tableId: string,
    @Param('recordId') recordId: string,
    @Param('fieldId') fieldId: string
  ): Promise<IRegenerateAiCellVo> {
    return await this.aiCellRegenerateService.regenerateAiCell(tableId, recordId, fieldId);
  }
