...
export class RecordOpenApiService {
  ...
  // regenerateAiCell + buildAiPromptForCell moved to AiCellRegenerateService
  // (apps/nestjs-backend/src/features/ai/ai-cell-regenerate.service.ts) to break
  // the runtime circular import that boot revealed: setting-open-api.service.ts
  // already imports from ai.service, so adding AiService DI here closed the cycle.
}
