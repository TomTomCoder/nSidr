...
export class LinkService {
  ...
  private async saveForeignKeyForManyMany(
    ...
    if (toAdd.length) {
      ...
      // One grouped query for all source records instead of one MAX() query each
      const maxOrders = field.getHasOrderColumn()
        ? await this.getMaxOrdersForTargets(
            fkHostTableName,
            selfKeyName,
            [...sourceGroups.keys()],
      ...
      for (const [sourceRecordId, targetRecordIds] of sourceGroups) {
        const currentMaxOrder = maxOrders[sourceRecordId] ?? 0;
  ...
  /**
   * Batched variant of getMaxOrderForTarget: one grouped query for all target records.
   * Records with no rows are absent from the result; callers should default to 0.
   */
  private async getMaxOrdersForTargets(
    tableName: string,
    keyColumn: string,
    ...
    const query = this.knex(tableName)
      .select({ targetId: keyColumn })
      .max(`${orderColumnName} as maxOrder`)
      .whereIn(keyColumn, targetRecordIds)
      .groupBy(keyColumn)
      .toQuery();

    const rows = await this.dataPrismaService
      .txClient()
      .$queryRawUnsafe<{ targetId: string; maxOrder: unknown }[]>(query);

    return rows.reduce<Record<string, number>>((acc, row) => {
      acc[row.targetId] = row.maxOrder == null ? 0 : Number(row.maxOrder);
      return acc;
    }, {});
  }
  ...
  private async saveForeignKeyForManyOne(
    ...
    if (toAdd.length) {
      ...
      // One grouped query for all target records instead of one MAX() query each
      const maxOrders = field.getHasOrderColumn()
        ? await this.getMaxOrdersForTargets(
            fkHostTableName,
            foreignKeyName,
      ...
      for (const [foreignRecordId, recordIds] of targetGroups) {
        const currentMaxOrder = maxOrders[foreignRecordId] ?? 0;
