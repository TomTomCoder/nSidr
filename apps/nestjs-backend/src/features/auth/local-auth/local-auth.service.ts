...
export class LocalAuthService {
  ...
  private async throwIfSystemEmail(email: string) {
    const normalized = email.toLowerCase();
    // All system users share the @system.teable.ai domain
    const isSystemDomain = normalized.endsWith('@system.teable.ai');
    if (isSystemDomain) {
      throw new CustomHttpException(
        `User ${email} is system user`,
    ...
    // Also catch system users identified by the isSystem DB flag
    const anyUser = await this.prismaService.user.findUnique({
      where: { email: normalized },
      select: { isSystem: true, email: true },
    });
    if (anyUser?.isSystem) {
      throw new CustomHttpException(
        `User ${anyUser.email} is system user`,
        HttpErrorCode.VALIDATION_ERROR,
        {
          localization: {
            i18nKey: 'httpErrors.auth.systemUser',
  ...
  async signup(body: ISignup, remoteIp?: string) {
    ...
    await this.throwIfSystemEmail(email);
