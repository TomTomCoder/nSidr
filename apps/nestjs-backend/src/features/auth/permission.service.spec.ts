...
describe('PermissionService', () => {
  ...
  describe('getEffectiveBaseRole (D-17.1-02 bug-4 fix)', () => {
    it('returns the base role when getRoleByBaseId is non-null (preserves existing semantics)', async () => {
      const baseId = 'base-id';
      const roleName = 'editor';
      prismaServiceMock.collaborator.findMany.mockResolvedValue([{ roleName } as any]);
      const result = await service.getEffectiveBaseRole(baseId);
      expect(result).toBe(roleName);
      // base.findFirst must NOT have been called when base role is present
      expect(prismaServiceMock.base.findFirst).not.toHaveBeenCalled();
    });

    it('inherits the space role when no base-collaborator row exists', async () => {
      const baseId = 'base-id';
      const spaceId = 'space-id';
      // First call (getBaseCollaborators) returns empty; second call (getSpaceCollaborators)
      // returns a space owner. The findMany mock is shared so we use mockImplementation
      // that toggles by call order.
      let callCount = 0;
      prismaServiceMock.collaborator.findMany.mockImplementation(async () => {
        callCount++;
        return callCount === 1 ? [] : ([{ roleName: 'owner' }] as any);
      });
      prismaServiceMock.base.findFirst.mockResolvedValue({ spaceId } as any);
      prismaServiceMock.space.findFirst.mockResolvedValue({ deletedTime: null } as any);
      const result = await service.getEffectiveBaseRole(baseId);
      expect(result).toBe('owner');
      expect(prismaServiceMock.base.findFirst).toHaveBeenCalledWith({
        where: { id: baseId },
        select: { spaceId: true },
      });
    });

    it('returns null when neither base nor space collaborator row exists', async () => {
      const baseId = 'base-id';
      prismaServiceMock.collaborator.findMany.mockResolvedValue([]);
      prismaServiceMock.base.findFirst.mockResolvedValue({ spaceId: 'space-id' } as any);
      prismaServiceMock.space.findFirst.mockResolvedValue({ deletedTime: null } as any);
      const result = await service.getEffectiveBaseRole(baseId);
      expect(result).toBeNull();
    });

    it('returns null when the base does not exist (does not throw)', async () => {
      const baseId = 'bogus-base';
      prismaServiceMock.collaborator.findMany.mockResolvedValue([]);
      prismaServiceMock.base.findFirst.mockResolvedValue(null as any);
      const result = await service.getEffectiveBaseRole(baseId);
      expect(result).toBeNull();
    });
  });
