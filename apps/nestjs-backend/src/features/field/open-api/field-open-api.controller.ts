...
export class FieldOpenApiController {
  ...
  private async assertFieldUpdatable(tableId: string, fieldId: string): Promise<void> {
    const table = await this.prismaService.tableMeta.findFirst({
      where: { id: tableId },
      select: { baseId: true },
    });
    if (!table) return;
    const ok = await this.fieldPermissionService.check(fieldId, 'field|update', table.baseId);
    if (!ok)
      throw new ForbiddenException(`field|update denied by field-level override for ${fieldId}`);
  }
  ...
  async updateField(
    ...
    await this.assertFieldUpdatable(tableId, fieldId);
    if (this.cls.get('useV2')) {
