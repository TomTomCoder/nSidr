...
export class FieldService implements IReadonlyAdapterService {
  ...
  private async handleDependentFormulaFields(
    ...
    try {
      ...
      // Batch-fetch all dependent field raws to avoid N+1 per-field queries
      const dependentFieldIds = fieldsToProcess.map(({ id }) => id);
      const dependentFieldRaws = await this.prismaService.txClient().field.findMany({
        where: { id: { in: dependentFieldIds }, deletedTime: null },
      });
      const fieldRawMap = new Map(dependentFieldRaws.map((r) => [r.id, r]));

      // Batch-fetch all distinct tableIds for tableMeta to avoid N+1 per-table queries
      const distinctTableIds = [...new Set(fieldsToProcess.map(({ tableId }) => tableId))];
      const tableMetas = await this.prismaService.txClient().tableMeta.findMany({
        where: { id: { in: distinctTableIds } },
        select: { id: true, dbTableName: true },
      });
      const tableMetaMap = new Map(tableMetas.map((t) => [t.id, t]));

      // Process each dependent formula field using batched lookups
      for (const { id: dependentFieldId, tableId: dependentTableId } of fieldsToProcess) {
        const dependentFieldRaw = fieldRawMap.get(dependentFieldId);
        ...
        // Get table name from pre-fetched map
        const dependentTableMeta = tableMetaMap.get(dependentTableId);
