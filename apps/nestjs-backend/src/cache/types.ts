...
export interface ICacheStore {
  ...
  // Performance caches — Plan 03-02
  // Table schema cache — keyed as "tableSchema:{tableId}"
  [key: `tableSchema:${string}`]: ITableSchemaCache;
  // Base config cache — keyed as "baseConfig:{baseId}"
  [key: `baseConfig:${string}`]: IBaseConfigCache;
...
export interface ITableSchemaCache {
  id: string;
  name: string;
  dbTableName: string;
  baseId: string;
...
export interface IBaseConfigCache {
  id: string;
  name: string;
  spaceId: string;
}
