...
function compileWorkerFile() {
  ...
  buildSync({
    ...
    platform: 'node',
    target: 'node20',
    // Native .node binaries cannot be bundled by esbuild — mark them external
    // so the compiled worker loads them at runtime via Node's native require.
    external: ['*.node', 'onnxruntime-node', '@xenova/transformers', 'sharp', '@vscode/sqlite3'],
  });
}
