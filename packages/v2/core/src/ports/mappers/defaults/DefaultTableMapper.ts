...
import { GanttView } from '../../../domain/table/views/types/GanttView';
...
class ViewToPersistenceVisitor implements IViewVisitor<ITableViewPersistenceDTO> {
  ...
  visitGanttView(view: GanttView): Result<ITableViewPersistenceDTO, DomainError> {
    return this.toDto(view, 'gantt');
  }
    ...
    return ViewId.create(dto.id).andThen((id) =>
      ViewName.create(dto.name).andThen((name) => {
        const viewResult = match(dto.type)
          ...
          .with('gantt', () => GanttView.create({ id, name }))
