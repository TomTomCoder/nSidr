...
class FakeTableRepository implements ITableRepository {
  ...
  async delete(_context: IExecutionContext, table: Table): Promise<Result<void, DomainError>> {
...
describe('CreateTableHandler', () => {
  ...
  it('returns errors from repositories and event bus', async () => {
    ...
    // Compensation: the committed metadata row must be removed, otherwise the
    // table is visible in the UI but unusable (physical table missing).
    expect(tableRepository.deleted.length).toBe(1);
