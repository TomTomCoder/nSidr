  ...
  async handle(
    ...
    return safeTry<CreateTableResult, DomainError>(async function* () {
      ...
      if (dataResult.isErr()) {
        ...
        // Compensate the already-committed meta transaction: without this, the
        // table appears in the UI but every query on it fails with 500 (its
        // physical table was never created) and there is no recovery path.
        // The table is seconds old and owns no user data, so permanent delete
        // is safe. Best-effort: if cleanup itself fails, the failed schema
        // operation above still records the inconsistency.
        const cleanupResult = await handler.unitOfWork.withTransaction(
          context,
          async (cleanupContext) =>
            handler.tableRepository.delete(cleanupContext, persistedTable, {
              mode: 'permanent',
            }),
          { scope: 'meta' }
        );
        if (cleanupResult.isErr()) {
          handler.logger.warn(
            `Failed to clean up metadata for table ${persistedTable
              .id()
              .toString()} after data-transaction failure: ${cleanupResult.error.message}`
          );
        }
