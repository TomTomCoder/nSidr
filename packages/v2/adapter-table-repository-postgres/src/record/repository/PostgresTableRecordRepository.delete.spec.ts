...
// Lets a test override how many rows a business-table DELETE actually removed,
// independent of how many rows the pre-delete SELECT matched. Returning a value
// < the selected count simulates a concurrent delete that removed the row first.
type DeletedRowsResolver = (compiledQuery: CompiledQuery) => bigint | undefined;

type RecordingSessionState = {
  undoBatchId?: string;
  // Rows returned by the most recent pre-delete record-id SELECT. Used as the
  // default numAffectedRows for the following DELETE (i.e. "all targets removed").
  lastDeleteTargetCount?: number;
};

const isBusinessTableDelete = (sql: string): boolean =>
  /^\s*delete\s+from/i.test(sql) && !sql.includes('__undo_log');
...
class RecordingConnection implements DatabaseConnection {
  constructor(
    ...
    private readonly deletedRowsResolver?: DeletedRowsResolver
  ) {}
  ...
  async executeQuery<R>(compiledQuery: CompiledQuery): Promise<QueryResult<R>> {
    ...
    if (isBusinessTableDelete(compiledQuery.sql)) {
      const numAffectedRows =
        this.deletedRowsResolver?.(compiledQuery) ??
        BigInt(this.sessionState.lastDeleteTargetCount ?? 0);
      return { rows: [] as R[], numAffectedRows };
    }

    const rows = (this.rowProvider?.(compiledQuery) ?? []) as R[];
    // Track the count of the pre-delete record-id SELECT so the subsequent DELETE
    // can report a realistic numAffectedRows by default.
    if (compiledQuery.sql.includes('select "__id" as "record_id"')) {
      this.sessionState.lastDeleteTargetCount = rows.length;
    }
    return { rows };
  }
...
const createRecordingDb = (
  ...
  deletedRowsResolver?: DeletedRowsResolver
) => {
...
describe('PostgresTableRecordRepository.deleteMany', () => {
      ...
      [
        ...
        },
        {
          ...
          "sql": "update "public"."table_meta" set "last_modified_time" = CASE
                WHEN "last_modified_time" IS NULL THEN (now() AT TIME ZONE 'utc')
                ELSE GREATEST((now() AT TIME ZONE 'utc'), "last_modified_time" + interval '1 millisecond')
  ...
  it('treats a target deleted concurrently as success, not a capture failure', async () => {
    // Regression: a record that vanished between our pre-delete SELECT and the
    // DELETE (e.g. a double-fired delete from the expand-record header) used to
    // surface as "Failed to capture complete delete snapshots. Expected 1, got 0".
    // The DELETE removed 0 rows, so the undo trigger captured nothing — which is
    // correct, not corruption. deleteMany must succeed with no captured snapshots.
    vi.useFakeTimers();
    vi.setSystemTime(new Date('2025-01-01T00:00:00.000Z'));
    ...
    const tableId = TableId.create(TABLE_ID)._unsafeUnwrap();
    const nameFieldId = FieldId.create(NAME_FIELD_ID)._unsafeUnwrap();
    // Target a record id other than RECORD_ID so the mock's default undo-log row
    // (keyed to RECORD_ID) is filtered out by record-id grouping — modelling an
    // undo log that captured nothing because the DELETE removed nothing.
    const recordId = RecordId.create(RECORD_ID_B)._unsafeUnwrap();
    const actorId = ActorId.create(ACTOR_ID)._unsafeUnwrap();
    ...
    const tableName = `"bse${'a'.repeat(16)}"."tbl${'b'.repeat(16)}"`;
    // Pre-delete SELECT still sees the row, but the DELETE removes 0 rows (a
    // concurrent request deleted it first) and the undo log captures nothing.
    const { db } = createRecordingDb(
      createRecordIdRowProvider(tableName, [recordId.toString()]),
      (compiledQuery) => (compiledQuery.parameters.includes(recordId.toString()) ? 0n : undefined)
    );
    const repo = createRepository(db, table);

    const result = await repo.deleteMany({ actorId }, table, deleteSpec);
    expect(result.isOk()).toBe(true);
    expect(result._unsafeUnwrap()).toEqual({ deletedRecords: [] });
