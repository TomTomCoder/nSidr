    ...
    return safeTry<DeleteManyResult, DomainError>(
      async function* (this: PostgresTableRecordRepository) {
        ...
        try {
          ...
          const deleteExecResult = await db
            .deleteFrom(tableName)
            .where(whereExpression)
            .executeTakeFirst();
          // Rows actually removed by THIS statement. May be < recordIds.length when a
          // concurrent request already deleted some of the targeted rows between our
          // pre-delete SELECT and this DELETE (e.g. a double-fired delete from the
          // expand-record header). Those rows are legitimately gone, not a capture gap.
          const actualDeletedCount = Number(deleteExecResult?.numDeletedRows ?? 0n);
          const mutationRows = yield* await snapshotCaptureSession.finish();
          ...
          if (actualDeletedCount < recordIds.length) {
            // Concurrency, not corruption: some targets vanished before our DELETE ran.
            this.logger.info('record:delete:already_removed', {
              tableId: table.id().toString(),
              expectedCount: recordIds.length,
              actualDeletedCount,
              capturedCount: deletedSnapshots.length,
            });
          }
          // The real invariant: every row this statement deleted must have been captured.
          // A shortfall here means the undo trigger failed to fire for a row we deleted.
          if (deletedSnapshots.length !== actualDeletedCount) {
            this.logger.warn('record:snapshot:missing', {
              operation: 'delete',
              tableId: table.id().toString(),
              recordIds: recordIdStrings,
              expectedCount: actualDeletedCount,
  ...
  private async touchTableMeta(
    ...
    await db
      ...
      .set({
        ...
        // (now() AT TIME ZONE 'utc'): the column is timestamp WITHOUT time zone and
        // the v1 read path (Prisma) interprets it as UTC — CURRENT_TIMESTAMP would
        // store local wall time and read back hours in the future.
