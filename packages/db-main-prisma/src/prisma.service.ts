  ...
  implements OnModuleInit
{
  ...
  constructor(
    ...
    const slowQueryThreshold = Number(process.env.PRISMA_SLOW_QUERY_THRESHOLD_MS ?? 0);
    const logConfig = {
      log: [
        ...(process.env.NODE_ENV !== 'production' || slowQueryThreshold > 0
    ...
    const initialConfig =
      process.env.NODE_ENV === 'production' && slowQueryThreshold <= 0 ? {} : { ...logConfig };
  ...
  async onModuleInit() {
    ...
    // Slow-query logging: enabled in any env via PRISMA_SLOW_QUERY_THRESHOLD_MS (>0).
    const slowQueryThreshold = Number(process.env.PRISMA_SLOW_QUERY_THRESHOLD_MS ?? 0);
      ...
      if (slowQueryThreshold > 0) {
        this.$on('query', (e) => {
          if (e.duration >= slowQueryThreshold) {
            this.logger.warn(
              `[slow query] ${e.duration}ms ${e.query.trim().replace(/\s+/g, ' ').slice(0, 2000)}`
            );
          }
        });
      }
    ...
    this.$on('query', (e) => {
      if (slowQueryThreshold > 0 && e.duration >= slowQueryThreshold) {
        this.logger.warn(`[slow query] ${e.duration}ms ${e.query.trim().replace(/\s+/g, ' ')}`);
        return;
      }
      this.logger.debug({
        Query: e.query,
