...
export const modelConfigSchema = z.object({
  ...
  // === Per-model API key override (GW-05, D-15-03 cascade) ===
  // Cascade: modelConfigs[model].apiKey → provider.apiKey → THROW
...
export const aiConfigSchema = z.object({
  ...
  // === Separate embedding provider config (GW-04, D-15-04) ===
  // Uses the same D-15-03 cascade: modelConfigs[modelId].apiKey → provider.apiKey → THROW
  embeddingProvider: z
    .object({
  ...
  // Ordered list of fallback modelKey strings tried after primary 429/5xx (ARH-01)
