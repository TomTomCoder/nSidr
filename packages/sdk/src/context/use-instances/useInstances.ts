...
// Realtime op logging is verbose (full doc-id arrays on every change) — opt in via
...
// A `destroyTimer` lets us defer teardown when refCount drops to 0, so a quick
// switch back to a previously-visited table reuses the live ShareDB subscription
// instead of paying the (~1s of CPU + WS round-trip) cost to re-subscribe.
type CachedQuery = { query: Query; refCount: number; destroyTimer?: ReturnType<typeof setTimeout> };
const subscribeQueryCache = new Map<string, CachedQuery>();
const SUBSCRIPTION_LINGER_MS = 30_000;
...
const acquireQuery = <T>(
  ...
  if (cached) {
    cached.refCount += 1;
    // Cancel pending destroy — caller is reusing the still-live subscription.
...
const releaseQuery = (key?: string, cb?: () => void) => {
  ...
  if (cached.refCount <= 0) {
    // Defer destroy so a quick re-acquire (e.g., user switching back to a
    // table they were just on) reuses the live ShareDB subscription instead
    // of paying ~1s of CPU + a WebSocket round-trip to re-subscribe.
    if (cached.destroyTimer) clearTimeout(cached.destroyTimer);
    cached.destroyTimer = setTimeout(() => {
      // Re-check refCount in case something re-acquired during the timer.
      const stillCached = subscribeQueryCache.get(key);
      if (!stillCached || stillCached.refCount > 0) return;
      subscribeQueryCache.delete(key);
      queryDestroy(stillCached.query);
...
export function useInstances<T, R extends { id: string }>({
  ...
  const [instances, dispatch] = useReducer(
    ...
    {
      // Always seed from SSR initData when present. The 'clear' reducer case
      // protects doc-less instances from being wiped on reconnect, and the
      // 'ready' case short-circuits when ShareDB confirms the same id set.
      // The prior `!connected` guard threw the SSR payload away whenever the
      // ShareDB connection was warm at hydration, forcing an empty→populated
      // flash and a full grid re-render once 'ready' fired.
  ...
  // True when collection just changed and ShareDB hasn't responded yet — safe to seed from cache.
  const awaitingShareDBRef = useRef(false);
  ...
  const handleReady = useCallback((query: Query<T>) => {
    ...
    debugLog('extra ready ->', query.extra);
    if (!query.results) {
      return;
    }
    awaitingShareDBRef.current = false;
    dispatch({ type: 'ready', results: query.results, extra: query.extra });
    query.results.forEach((doc) => {
      opListeners.current.add(doc, (op) => {
        debugLog(`${query.collection} on op:`, op, doc);
        dispatch({ type: 'update', doc });
      });
    });
  }, []);
  ...
  const handleInsert = useCallback((docs: Doc<T>[], index: number) => {
    ...
    docs.forEach((doc) => {
      opListeners.current.add(doc, (op) => {
        debugLog(`${docs[0]?.collection} on op:`, op);
        dispatch({ type: 'update', doc });
      });
    });
  }, []);
  ...
  useEffect(() => {
    ...
    if (shouldClearInstances) {
      awaitingShareDBRef.current = true;
      if (initData?.length) {
        // Seed immediately from cached/prefetched data so the UI renders without waiting
        // for the ShareDB round-trip. ShareDB 'ready' will replace these with live docs.
        dispatch({ type: 'initSeed', data: initData });
      } else {
        dispatch({ type: 'clear' });
      }
      setQuery(undefined);
  ...
  // If prefetch data arrives after collection changes (user clicks before hover resolves),
  // seed immediately rather than waiting for the ShareDB round-trip.
  // eslint-disable-next-line react-hooks/exhaustive-deps
  useEffect(() => {
    if (initData?.length && awaitingShareDBRef.current) {
      dispatch({ type: 'initSeed', data: initData });
    }
  }, [initData]); // intentionally omits dispatch (stable) and awaitingShareDBRef (ref)
