...
export function getWsPath() {
  ...
  //
  // Prefer NEXT_PUBLIC_SOCKET_URL when set. Required for local prod runs
  // (`pnpm start:prod:local`) because Next.js rewrites cannot proxy the
  // SockJS/WebSocket upgrade — they only handle plain HTTP. Without this
  // env, /socket/info falls back to long-polling, retries with backoff,
  // and pegs the main thread for 20+ s on every page that mounts ShareDB.
  // In real deployments a reverse proxy (nginx/Caddy/traefik) terminates
  // the WS upgrade and same-origin works correctly; leave the env unset.
  const envUrl =
