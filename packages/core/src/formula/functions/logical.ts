...
export class IsBlank extends LogicalFunc {
  name = FunctionName.IsBlank;
  acceptValueType = new Set([CellValueType.String, CellValueType.Number, CellValueType.Boolean, CellValueType.DateTime]);
  acceptMultipleValue = false;
  validateParams(p: TypedValue[]) { if (p.length !== 1) throw new Error(`${FunctionName.IsBlank} needs 1 param`); }
  getReturnType() { return { type: CellValueType.Boolean }; }
  eval(p: TypedValue[]) { const v = p[0].value; return v == null || v === '' || (Array.isArray(v) && v.length === 0); }
}

export class IfBlank extends LogicalFunc {
  name = FunctionName.IfBlank;
  acceptValueType = new Set([CellValueType.String, CellValueType.Number, CellValueType.Boolean, CellValueType.DateTime]);
  acceptMultipleValue = false;
  validateParams(p: TypedValue[]) { if (p.length !== 2) throw new Error(`${FunctionName.IfBlank} needs 2 params`); }
  getReturnType(p?: TypedValue[]) { return p ? { type: p[1].type } : { type: CellValueType.String }; }
  eval(p: TypedValue[]) { const v = p[0].value; return (v == null || v === '') ? p[1].value : v; }
}

export class IfError extends LogicalFunc {
  name = FunctionName.IfError;
  acceptValueType = new Set([CellValueType.String, CellValueType.Number, CellValueType.Boolean, CellValueType.DateTime]);
  acceptMultipleValue = false;
  validateParams(p: TypedValue[]) { if (p.length !== 2) throw new Error(`${FunctionName.IfError} needs 2 params`); }
  getReturnType(p?: TypedValue[]) { return p ? { type: p[1].type } : { type: CellValueType.String }; }
  eval(p: TypedValue[]) {
    const v = p[0].value;
    return (v instanceof Error || (typeof v === 'string' && v.startsWith('#'))) ? p[1].value : v;
  }
}
...
export class IsNumber extends LogicalFunc {
  name = FunctionName.IsNumber;
  acceptValueType = new Set([CellValueType.String, CellValueType.Number, CellValueType.Boolean, CellValueType.DateTime]);
  acceptMultipleValue = false;
  validateParams(p: TypedValue[]) { if (p.length !== 1) throw new Error(`${FunctionName.IsNumber} needs 1 param`); }
  getReturnType() { return { type: CellValueType.Boolean }; }
  eval(p: TypedValue[]) { return p[0].type === CellValueType.Number; }
}

export class IsText extends LogicalFunc {
  name = FunctionName.IsText;
  acceptValueType = new Set([CellValueType.String, CellValueType.Number, CellValueType.Boolean, CellValueType.DateTime]);
  acceptMultipleValue = false;
  validateParams(p: TypedValue[]) { if (p.length !== 1) throw new Error(`${FunctionName.IsText} needs 1 param`); }
  getReturnType() { return { type: CellValueType.Boolean }; }
  eval(p: TypedValue[]) { return p[0].type === CellValueType.String; }
}

export class IsDate extends LogicalFunc {
  name = FunctionName.IsDate;
  acceptValueType = new Set([CellValueType.String, CellValueType.Number, CellValueType.Boolean, CellValueType.DateTime]);
  acceptMultipleValue = false;
  validateParams(p: TypedValue[]) { if (p.length !== 1) throw new Error(`${FunctionName.IsDate} needs 1 param`); }
  getReturnType() { return { type: CellValueType.Boolean }; }
  eval(p: TypedValue[]) { return p[0].type === CellValueType.DateTime; }
}

export class IsNaN extends LogicalFunc {
  name = FunctionName.IsNaN;
  acceptValueType = new Set([CellValueType.Number, CellValueType.String]);
  acceptMultipleValue = false;
  validateParams(p: TypedValue[]) { if (p.length !== 1) throw new Error(`${FunctionName.IsNaN} needs 1 param`); }
  getReturnType() { return { type: CellValueType.Boolean }; }
  eval(p: TypedValue[]) { return Number.isNaN(p[0].value); }
}

export class IsFinite extends LogicalFunc {
  name = FunctionName.IsFinite;
  acceptValueType = new Set([CellValueType.Number]);
  acceptMultipleValue = false;
  validateParams(p: TypedValue[]) { if (p.length !== 1) throw new Error(`${FunctionName.IsFinite} needs 1 param`); }
  getReturnType() { return { type: CellValueType.Boolean }; }
  eval(p: TypedValue<number | null>[]) { const v = p[0].value; return v == null ? false : Number.isFinite(v); }
}
