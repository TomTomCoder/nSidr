...
export class CreatedBy extends SystemFunc {
  name = FunctionName.CreatedBy;
  acceptValueType = new Set([CellValueType.String]);
  acceptMultipleValue = false;
  validateParams() { return; }
  getReturnType() { return { type: CellValueType.String }; }
  eval(_p: TypedValue[], context: IFormulaContext) { return (context.record as Record<string, unknown>)['createdBy'] as string ?? null; }
}

export class LastModifiedBy extends SystemFunc {
  name = FunctionName.LastModifiedBy;
  acceptValueType = new Set([CellValueType.String]);
  acceptMultipleValue = false;
  validateParams() { return; }
  getReturnType() { return { type: CellValueType.String }; }
  eval(_p: TypedValue[], context: IFormulaContext) { return (context.record as Record<string, unknown>)['lastModifiedBy'] as string ?? null; }
}
